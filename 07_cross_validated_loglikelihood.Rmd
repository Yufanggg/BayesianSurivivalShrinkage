...
cross_validated_likelihood <- function(strafied_Data_int){
  cvl_s = rep(NA, nrow(strafied_Data_int))
  for (i in 1:nrow(strafied_Data_int)){
    new_strafied_Data_int <- strafied_Data_int[-i, ]
    stan_data_bSpline <- stan_data_Constructer_BSpline(
      training_dataset = new_strafied_Data_int,
      testing_dataset = NULL,
      obs_window = 3415
    )
    model_fit_bSpline <- Bayesian_Survival_model(stan_data = stan_data_bSpline, baseline_modelling = "bSplines", withPrediction = FALSE, shrinkage = TRUE)
    diagnostics_bSpline <- summary(model_fit_bSpline)$summary[, c("Rhat", "n_eff")]
    
    log_lik_matrix <- extract_log_lik(stanfit, parameter_name = "log_lik", merge_chains = TRUE) # S*N
    log_lik_n <- apply(log_lik_matrix, 2, sum)
    cvl_s[i] <- max(log_lik_n)
  }
  return(sum(cvl_s))
}
