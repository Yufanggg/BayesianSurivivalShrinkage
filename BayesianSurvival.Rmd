...
Bayesian_Survival_baseline <- function(stan_data, assumption = "exponential", school = "Bayesian", 
  ...
  if (school == "Bayesian") {
    ...
    for (i in 1:N){
      target += exponential_lpdf(t[i] | lambda * exp(x*Beta));
    } // uncensored data log(f(t))

    for (i in 1:N_cens){
       target += exponential_lccdf(t_cens[i] | lambda * exp(x*Beta));
    } // right censored data log(S(t))
    ...
    "
     ...
     model {
       ...
       // log-likelihood, represented by [target]
       for (i in 1:N){
         target += weibull_lpdf(t[i] | alpha, sigma/exp(x*Beta/alpha)); // weibull_lpdf() is at log scale
        } // uncensored data log(f(t))

       for (i in 1:N_cens){
          target += weibull_lccdf(t_cens[i] | alpha, sigma/exp(x_cens*Beta/alpha)); // weibull_lccdf() is at log scale
       } // right censored data log(S(t)) //log complementary cumulative distribution function (log-CCDF) for an weibull distribution
  }
"
    }
      ...
      bSpline_basis <- bSpline(x, knots = knots, degree = 3, intercept = FALSE) # The B-spline basis is calculated using the method implemented in the splines2 package
      # Compute the derivative of the B-spline basis
      bSpline_derivative <- deriv(bs_basis)
      ...
      parameter {
        vector[K] cofficients;
        real<lower=0> sigma;
      } 
        
      model {
      // Priors
      cofficients ~ normal(0, 1)
      sigma ~ lognormal(0, 1)
      
      // log-likelihood, represented by [target]
      for (i in 1:N){
         target += bSpline_basis*cofficients * exp(x * Beta) + log(cofficients) + x*Beta + log(bSpline_derivative)
         }; // uncensored data log(f(t))
        
      for (i in 1:N_cens){
          target -= bSpline_basis* cofficients * exp(x_cens * Beta);  \\ at log scale
      } // right censored data log(S(t)) //log complementary cumulative distribution function (log-CCDF) for an weibull distribution
          }
    "
    }
...
Output3 <- Bayesian_Survival_baseline(stan_data = stan_data_basline, assumption = "weibull") 
print(Output3)
...
Output3 <- Bayesian_Survival_baseline(stan_data = stan_data_kidney_basline, assumption = "exponential") 
print(Output3)
```
