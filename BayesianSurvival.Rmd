      ...
      functions {
        real gauss_kronrod_quad_Hazard_basline(real[] locat, real[] weights, real Ti, vector T, matrix bSpline_basis, vector coefficients){
          ...
          for (q in 1:num_elements(locat)){
            u = Ti * (1 + locat[q])/2;
            integral_ += weights[q] * exp(logh_0i(u, T, bSpline_basis, coefficients));
          }
        ...
        real logh_0i(real u, vector T, matrix bSpline_basis, vector coefficients){
           ...
           int index;
           vector[num_elements(T)] logh_0t;
           real logh_0i = 0;
           logh_0t = bSpline_basis * coefficients + sigma;
           index = find_index(u, T);
           if (index != -1){
             
             logh_0i = logh_0t[index];
           }
           else { // if u not found in T, use linear interpolation
             int lower_index = 1;
             int upper_index = num_elements(T);
             
             // find the cloest indices for interpolation
             for (i in 1:(num_elements(T)-1)){
               if (T[i] <= u && u < T[i + 1]) {
                    lower_index = i;
                    upper_index = i + 1;
                    break;
