...
- LogLikihood function =  sum(-H_i + logh_i) for uncensored data + sum(-H_i) for censored data.

## Evaluation techniques
- The **prediction** is estimated at the level of Intergal brier score. In the case of testing dataset and training dataset were observed at different time points, we used the linear interpolation to get the estimated survival probability at the time points that only occurs in the testing dataset.
...
CrossValidation <- function(dataset = sim_data, KFold = 5, FDR = 0.1, Beta_real = beta, simulated = True){
  ...
  rMSE = matrix(NA, nrow = KFold, ncol = length(!(names(dataset) %in% c("id", "eventtime", "status", "obstime", "status1"))) )
  ...
  for (i in 1:KFold){
    ...
    if (simulated == TRUE){
      ...
      sp_matrix = matrix(sp, nrow = nrow(test_dataset))
