...
CrossValidation <- function(dataset = sim_data, KFold = 5){
  ...
  for (i in 1:KFold){
    ...
    stan_data <- list(
      ...
      t = train_dataset[train_dataset$status == 1, "eventtime"],
      ...
      t_cens = train_dataset[train_dataset$status == 0, "eventtime"],
      x_cens = train_dataset[train_dataset$status == 0,!(names(train_dataset) %in% c("eventtime", "status"))],
    ...
    Output[grep("^Beta", rownames(Output)), "mean", drop = FALSE] ## Extract the Betas
