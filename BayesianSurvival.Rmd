...
"
      ...
      functions {
        real gauss_kronrod_quad_Hazard_basline(real Ti, vector T, matrix bSpline_basis, vector coefficients){
            ...
            real  [15] = {0.022935, 0.063093, 0.104790, 0.140653, 0.169004, 0.190350, 0.204432, 0.209482, 0.204432, 0.190350, 0.169004, 0.140653, 0.104790, 0.063093, 0.022935};
...
stan_data_includingbaseline <- list(
  N = stan_data$N,
  t = stan_data$t,
...
Output3 <- Bayesian_Survival_includingbaseline(stan_data = stan_data_includingbaseline, baseline_assumption = "bSplines") #exponential, weibull,  bSplines
print(Output3)
```
...
covariates <- data.frame(
  id = 1:1000,  # 100 individuals
  cov1 = rbinom(1000, 1, 0.5),  # First covariate
  cov2 = rnorm(1000, mean = 0, sd = 1)   # Second covariate
)
...
sim_data = merge(covariates, sim_data, by = "id")
# Define the cross-validation
CrossValidation <- function(dataset = sim_data, KFold = 5){
  folds <- cut(seq(1:nrow(dataset)), break = KFold, labels = FALSE)
  for (i in 1:KFold){
    ...
    stan_data <- list(
      ...
      k = ncol(train_dataset) - 2,
      # 2 represents the columns of t and status
      N = nrow(train_dataset[status == 1,]),
      # Number of non-censored data
      t = train_dataset[status == 1, "eventtime"],
      x = train_dataset[train_dataset$status == 1, !(names(train_dataset) %in% c("eventtime", "status"))],
      
      N_cens = nrow(train_dataset[status == 0,]),
      # Number of censored data
      t_cens = train_dataset[status == 0, "eventtime"],
      x_cens = train_dataset[status == 0,!(names(train_dataset) %in% c("eventtime", "status"))],
      ...
      x_new = test_dataset[,!(names(test_dataset) %in% c("eventtime", "status"))],
      t_new = train_dataset[, "eventtime"],
      status_new = train_dataset[, "status"]
    )
    ...
    Output3["Beta[1]", "mean"] # Extract the Betas
    message("In the case of real-world data, we used the Cox regression estimator as real Beta")
    cox_model = coxph(Surv(train_dataset$eventtime, train_dataset$status) ~ .)
    
    # evaluating the model performance at the level of prediction
    survival_prob = matrix(Output[grep("^survival_prob", rownames(Output)), "mean", drop = FALSE], nrow = nrow(test_dataset))
