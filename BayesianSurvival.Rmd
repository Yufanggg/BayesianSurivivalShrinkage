...
"
      ...
      code_bayesian_model <- "
      functions {
        real gauss_kronrod_quad_Hazard_basline(real[] locat, real[] weights, real Ti, vector T, matrix bSpline_basis, vector coefficients){
    ...
    # Summary of the fit
    output <- summary(bayesian_model_fit)$summary
    output$stan_data <- output$stan_data
    return(output)
  }
  }
...
for (i in 1:folds) {
  ...
  k = ncol(train_dataset) - 2, # 2 represents the columns of t and status
  N = nrow(train_dataset[status == 1, ]), # Number of non-censored data
  t = train_dataset[status == 1, "t"],
  x = train_dataset[status == 1, -c("t", "status")],
  
  N_cens = nrow(train_dataset[status == 0, ]), # Number of censored data
  t_cens = train_dataset[status == 0, "t"],
  x_cens = train_dataset[status == 0, -c("t", "status")],
  ...
  x_new = test_dataset[, -c("t", "status")],
  t_new = train_dataset[, "t"],
  status_new = train_dataset[, "status"],
  
  # Data for Cox regression model
  covariate = stan_data$covariate,
  obstime = stan_data$obstime,
  status = stan_data$status
  )
