...
"
      ...
      functions {
        ...
        real h_i(real u, vector T, matrix bSpline_basis, vector coefficients, matrix x, vector beta){
           ...
           // Print the size of vectors and matrices 
           print(num_elements(T)); 
           print(rows(bSpline_basis));
           print(cols(bSpline_basis));
           print(num_elements(coefficients));
           print(num_elements(x));
           
           print(num_elements(h_t));
     ...
     int find_index(real u, vector t) {
      ...
      // log-likelihood, represented by [target]
      for (Ti in uniqueT){
        if (is_in_vector(Ti, t) == 1) {
         target += -gauss_kronrod_quad(locat, weights, Ti, uniqueT, bSpline_basis, coefficients, x, Beta);// + h_i(Ti, uniqueT, bSpline_basis, coefficients, x, Beta); // uncensored data log(f(t)) = -H(t) + h(t)
        }
        else if (is_in_vector(Ti, t_cens) == 1){
         target += -gauss_kronrod_quad(locat, weights, Ti, uniqueT, bSpline_basis, coefficients, x_cens, Beta);  // at log scale,  right censored data log(S(t)),  log(S(t)) = -H(t)
        }
      }
      }
    "
