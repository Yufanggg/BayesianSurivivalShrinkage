...
Bayesian_Survival_baseline <- function(stan_data, assumption = "exponential",
  ...
  parameters {
    real<lower = 0> shape; // parameters in weibull assumption
    real<lower = 0> scale; // parameters in weibull assumption
  }
  model {
    // Priors
    shape ~ lognormal(0, 1); 
    ...
    // log-likelihood, represented by [target]
    for (i in 1:N){
      target += weibull_lpdf(t[i] | alpha, sigma/exp(x*Beta/alpha))
    } // uncensored data log(f(t))
    
    for (i in 1:N_cens){
       target += weibull_lccdf(t_cens[i] | alpha, sigma/exp(x*Beta/alpha))
