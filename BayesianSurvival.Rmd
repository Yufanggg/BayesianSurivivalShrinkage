...
Bayesian_Survival_includingbaseline <- function(stan_data_baseline, assumption = "exponential", school = "Bayesian", 
                              niter = 10000, 
                              nwarmup = 1000,
                              thin = 10,
                              chains = 1) {
...
"
      ...
      time_combined <- sort(unique(c(stan_data_baseline$t, stan_data_baseline$t_cens)))
      time_cens <- sort(unique(stan_data_baseline$t_cens))
      time_ <- sort(unique(stan_data_baseline$t))
      ...
      stan_data_baseline$bSpline_basis <- bSpline_basis
      stan_data_baseline$M = length(time_combined)
      stan_data_baseline$uniqueT = time_combined
      stan_data_baseline$O = length(time_)
      stan_data_baseline$uniquet = time_
      stan_data_baseline$Q = length(time_cens)
      stan_data_baseline$uniquet_cens = time_cens
      
      code_bayesian_model <- "
      functions {
        real gauss_kronrod_quad_Hazard_basline(real[] locat, real[] weights, real Ti, vector T, matrix bSpline_basis, vector coefficients, vector sigma){

          // vectorize all computation
          vector[num_elements(locat)] u_vec = Ti * (rep_vector(1, num_elements(locat))/2);
          vector [num_elements(locat)] logh_0i_vec = vectorized_logh_0i(u_vec, T, bSpline_basis, coefficients, sigma);
          real integral_ = sum(to_vector(weights) .* exp(logh_0i_vec));
          
          return integral_/2;
        }
        
        vector vectorized_logh_0i(vector u_vec, vector T, matrix bSpline_basis, vector coefficients, vector sigma){
        ...
        real logh_0i(real u, vector T, matrix bSpline_basis, vector coefficients, vector sigma){
           // define the log baseline hazard function in a vectorized way
           vector[num_elements(T)] logh_0t = bSpline_basis * coefficients;
           real logh_0i;
     ...
     int find_index(real u, vector t) {
      ...
      // log-likelihood, represented by [target]
      for (Ti in uniqueT){
        real H_0i_ = gauss_kronrod_quad_Hazard_basline(locat, weights, Ti, uniqueT, bSpline_basis, coefficients);
        real logh_0i_ = logh_0i(Ti, uniqueT, bSpline_basis, coefficients);
        
        if (find_index(Ti, uniquet) != -1) {
         vector[N] H_i =   .* exp(x*Beta);
    ...
    generated quantities{ // predicting the survival time on the new/test dataset
      matrix[M, N_new] survival_prob; // unqiue time points * rows of new data
      for (m in 1:M){
        real H_0i_ = gauss_kronrod_quad_Hazard_basline(locat, weights, m, uniqueT, bSpline_basis, coefficients);
        survival_prob[m,] = exp(-H_0i_ + x_new*Beta) //vectorize over the x_new, output is the time points * obs
      }
    }
    "
    }
    ...
    # Summary of the fit
    output <- summary(bayesian_model_fit)$summary
    output$stan_data_baseline <- output$stan_data_baseline
    return(output)
  }
  }
...
Output3 <- Bayesian_Survival_includingbaseline(stan_data = stan_data_baseline, assumption = "bSplines") #exponential, weibull,  bSplines
print(Output3)
```
# evaluate the model performance:
```{r, echo=TRUE}
model_performance_eval = function(test_uniqueT, test_status, pred_survival_prob, simulated_data = FASLE){
  # pred_survival_prob: dimension is time_points*n_participants, prob in each cell
  # test_status: dimension is time_points*n_participants, binary numbers in each cell
  if (simulated_data == FALSE){
    message("with a non-simulated data, we can only evaluate the prediction performance")
    # using the Time-dependent Brier score for such prediction performance eval
    message("Time-dependent Brier score was used for model prediction preformance eval")
    
    Brier_score = mean(D - pred_survival_prob)
  }
