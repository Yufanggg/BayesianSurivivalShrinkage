...
generated quantities{
  ...
  for (n in 1:N){
    if (status[n] == 1){
      log_lik[n] = weibull_lpdf(t[n] | shape, 1/(exp(X[n] * Beta + X_int[n] * Beta_int) * lambda)); // uncensored data log(f(t)) = log(h(t)) + log(S(t))
    } else {
      log_lik[n] = weibull_lccdf(t[n] | shape, 1/(exp(X[n] * Beta + X_int[n] * Beta_int) * lambda)); // right censored data log(S(t))
    }
  }
}
