...
simulation_bayint_survival_shrinkage <- function(criteria = c("DesignCoefficients", "baseline", "variableSelection", "Prediction_SurvivalProb"), withPrediction = TRUE){
  ...
  #fit model
  model_fit_bSplines_withShrinkage <- Bayesian_Survival_model(stan_data = stan_data_bSplines, baseline_assumption = "bSplines")
  ...
  if ("variableSelection" %in% criteria) {
    output$model_result_bSplines_withShrinkage <-
...
for (i in 1:replicate_times){
  ...
  CI_index_bSplines[i] <- rcorr.cens(sp_pre_bSplines, Surv(time, true_status))[["C Index"]] # rcorr.cens
