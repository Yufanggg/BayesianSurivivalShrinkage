...
simulation_bayint_survival_shrinkage <- function(criteria = c("DesignCoefficients", "baseline", "variableSelection", "Prediction_SurvivalProb"), withPrediction = TRUE){
  ...
  if ("Prediction_SurvivalProb" %in% criteria) {
    output$time = tail(dataset$obstime, 100)
    output$true_status = tail(dataset$status, 100)
...
for (i in 1:replicate_times){
  ...
  MSEs_bSplines_noShrinkage[i, ] <- (Beta_est_bSplines_noShrinkage - Beta_true)**2
  ...
  FDR_bSplines_noShrinkage[i] <- sum(Selection_bSplines_noShrinkage & !variableSelection_reference) / sum(Selection_bSplines_withShrinkage)
  
  
  #Brier_score
  true_status <- simulated_results[,i]$true_status
  sp_pre_bSplines_withShrinkage <- simulated_results[,i]$sp_bSplines_withShrinkage
  Brier_score_bSplines_withShrinkage[i] <- (true_status - sp_pre_bSplines_withShrinkage)**2
  
  sp_pre_bSplines_noShrinkage <- simulated_results[,i]$sp_bSplines_noShrinkage
  Brier_score_bSplines_noShrinkage[i] <- (true_status - sp_pre_bSplines_noShrinkage)**2
  ...
  CI_index_bSplines_noShrinkage[i] <- rcorr.cens(sp_pre_bSplines_noShrinkage, Surv(time, true_status))[["C Index"]] 
