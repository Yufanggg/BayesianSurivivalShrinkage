...
par(mfrow = c(3, 1))
plot(combined_Data_int_noise_bSpline_model_result_coef, type = "o", col = "blue", xlab = "", ylab = "Beta", main = "Beta estimation", pch = 16, ylim = c(-5, 5), xaxt = "n")
...
par(mfrow = c(3, 1))
plot(combined_Data_int_noise_bSpline_model_Simple_result_coef, type = "o", col = "blue", xlab = "", ylab = "Beta", main = "Beta estimation", pch = 16, ylim = c(-5, 5), xaxt = "n")
# Add background rectangles
...
bSplines_model_Prediction_SurvivalProb_Simple = bSplines_model_Prediction_SurvivalProb = matrix(NA, nrow = k, ncol = nrow(combined_Data_int)/k)
coxRegression_SurvivalProb_Simple = coxRegression_SurvivalProb = matrix(NA, nrow = k, ncol = nrow(combined_Data_int)/k)
PL_Prediction_SurvivalProb_Simple = PL_Prediction_SurvivalProb = matrix(NA, nrow = k, ncol = nrow(combined_Data_int)/k)
...
for (i in seq_along(folds)) {
  ...
  bSplines_model_Prediction_SurvivalProb[i, ] <- as.vector(bSplines_model_results$eta_pred)
  ...
  bSplines_model_Prediction_SurvivalProb_Simple[i, ] <- bSplines_model_results_Simple$eta_pred

    #-----------MODEL 1.2: Cox regression ----------
    cox_regression <- coxph(Surv(as.numeric(obstime), status) ~ ., data = training_dataset)
    coxRegression_SurvivalProb[i,] <- predict(cox_regression, newdata = testing_dataset, type = "lp")
    
    #-----------MODEL 2.2: Cox regression ----------
    cox_regression_Simple <- coxph(Surv(as.numeric(obstime), status) ~ ., data = training_dataset_Simple)
    coxRegression_SurvivalProb_Simple[i,] <- predict(cox_regression_Simple, newdata = testing_dataset2_Simple, type = "lp")
    ...
    PL_Prediction_SurvivalProb[i, ] <- PL_results$eta_pred
    ...
    PL_Prediction_SurvivalProb_Simple[i, ] <- PL_results_Simple$eta_pred
}
