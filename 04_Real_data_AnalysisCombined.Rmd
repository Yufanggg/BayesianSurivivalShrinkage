...
par(mfrow = c(3, 1))
plot(combined_Data_int_noise_bSpline_model_Simple_result_coef, type = "o", col = "blue", xlab = "", ylab = "Beta", main = "Beta estimation", pch = 16, ylim = c(-10, 10), xaxt = "n")
# Add background rectangles
...
#axis(1, at = 1:91, labels = "", las = 2, cex.axis = 0.7)
axis(1, at = 1:108, labels = name_label_Simple, las = 2, cex.axis = 0.7)
abline(a = 0, b =0, col = "red")
...
bSplines_model_Prediction_SurvivalProb_Simple = bSplines_model_Prediction_SurvivalProb = matrix(NA, nrow = length(folds[[1]]), ncol = 105)
coxRegression_SurvivalProb_Simple = coxRegression_SurvivalProb = matrix(NA, nrow = length(folds[[1]]), ncol = 105)
PL_Prediction_SurvivalProb_Simple = PL_Prediction_SurvivalProb = matrix(NA, nrow = length(folds[[1]]), ncol = 105)


for (r in seq_along(folds)) {
  cat("Repetition", r, "\n")
  ...
  bSplines_model_Prediction_SurvivalProb[i, ] <- bSplines_model_results$Prediction_SurvivalProb
  ...
  bSplines_model_Prediction_SurvivalProb_Simple[i, ] <- bSplines_model_results_Simple$Prediction_SurvivalProb

    #-----------MODEL 1.2: Cox regression ----------
    cox_regression <- coxph(Surv(as.numeric(obstime), status) ~ ., data = training_dataset)
    cox_regression_predicted_result <- predict(cox_regression, newdata = testing_dataset, type = "risk")
    
    #-----------MODEL 2.2: Cox regression ----------
    cox_regression_Simple <- coxph(Surv(as.numeric(obstime), status) ~ ., data = training_dataset_Simple)
    cox_regression_predicted_result_Simple <- predict(cox_regression, newdata = testing_dataset_Simple, type = "risk")
    ...
    PL_Prediction_SurvivalProb[i, ] <- PL_results$Prediction_SurvivalProb
    ...
    PL_Prediction_SurvivalProb_Simple[i, ] <- PL_results_Simple$Prediction_SurvivalProb
}
...
for (r in 1:n_repeats) {
  ...
  bSplines_model_beta_Simple <- matrix(NA, nrow = k, ncol = p)
  bSplines_model_variableSelection_Simple <- matrix(NA, nrow = k, ncol = p)
  PL_beta_Simple <- matrix(NA, nrow = k, ncol = p)
  PL_variableSelection_Simple <- matrix(NA, nrow = k, ncol = p)
  cox_beta_Simple <- matrix(NA, nrow = k, ncol = p)
  cox_variableSelection_Simple <- matrix(NA, nrow = k, ncol = p)
  
  
  bSplines_model_beta2_Simple <- matrix(NA, nrow = k, ncol = p)
  bSplines_model_variableSelection2_Simple <- matrix(NA, nrow = k, ncol = p)
  PL_beta2_Simple <- matrix(NA, nrow = k, ncol = p)
  PL_variableSelection2_Simple <- matrix(NA, nrow = k, ncol = p)
  cox_beta2_Simple <- matrix(NA, nrow = k, ncol = p)
  cox_variableSelection2_Simple <- matrix(NA, nrow = k, ncol = p)
  ...
  for (i in seq_along(folds)) {
    ...
    cox_variableSelection[i,] <- as.vector((cox_regression_result$coefficients[, "Pr(>|z|)"] * 105) < 0.05)
    
    cox_beta2[i,] <- as.vector(cox_regression_result2$coefficients[, "coef"])
    cox_variableSelection2[i,] <- as.vector((cox_regression_result2$coefficients[, "Pr(>|z|)"] * 105 ) < 0.05)
    ...
    cox_variableSelection_Simple[i,] <- as.vector((cox_regression_result_Simple$coefficients[, "Pr(>|z|)"] * 105) < 0.05)
    
    cox_beta2_Simple[i,] <- as.vector(cox_regression_result2_Simple$coefficients[, "coef"])
    cox_variableSelection2_Simple[i,] <- as.vector((cox_regression_result2_Simple$coefficients[, "Pr(>|z|)"] * 105 ) < 0.05)
