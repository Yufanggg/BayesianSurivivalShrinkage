...
combined_Data_int_noise_cox_regression_result_variableSelection <- as.matrix((combined_Data_int_noise_cox_regression_result$coefficients[, "Pr(>|z|)"] *110) < 0.05)
...
folds <- createFolds(combined_Data_int$status, k = 2, list = TRUE, returnTrain = FALSE)

# Cross validation (5-fold)
for (i in seq_along(folds)){
  indices <- folds[[i]]
  training_data <- combined_Data_int[-indices, ]
  testing_data <- combined_Data_int[indices, ]
  
  cat("Fold", i, "\n")
  cat("training dataset size:", nrow(training_data), "\n")
  cat("testing dataset size:", nrow(testing_data), "\n")
  ...
  bSplines_model <- Bayesian_Survival_model(stan_data = Data_bSplines,
                                                baseline_modelling = "bSplines",
                                                shrinkage = TRUE,
                                                withPrediction = FALSE,
                                                havingInt = TRUE)
  ...
  cox_regression <- coxph(Surv(as.numeric(obstime), status) ~., data = training_data)
  cox_regression_result <- summary(cox_regression)
  cox_regression_result_coef <- as.matrix(combined_Data_int_noise_cox_regression_result$coefficients[, "coef"])
combined_Data_int_noise_cox_regression_result_variableSelection <- as.matrix((combined_Data_int_noise_cox_regression_result$coefficients[, "Pr(>|z|)"] *110) < 0.05)
  ...
  # Extract the result from the PL model
  
  PL_model_results = Bayesian_Survival_result_Extract(
  ...
  PL_beta[i,] <- PL_model_results$Beta_bayesian_est[, "mean"]
  PL_variableSelection[i,] <- PL_model_results$variableSelection
}
...
save.image(file = "./Data/03_results_combined_Step5.RData")
```
