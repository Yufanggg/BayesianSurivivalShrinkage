...
simulation_bayint_survival <- function(criteria = c("DesignCoefficients", "baseline", "variableSelection"), n_samples = n_samples, n_features = n_features){
  ...
  cvl_exponential <- cross_validated_log_likelihood (model_fit_exponential, n_samples)
  cvl_weibull <- cross_validated_log_likelihood (model_fit_weibull, n_samples)
  cvl_bSplines <- cross_validated_log_likelihood (model_fit_bSplines, n_samples)
...
for (i in 1:replicate_times){
  ...
  Variance_PH_[i, ] = simulated_results[,i]$Beta_est_PH - Beta_true
  ...
  MSEs_bSplines_[i, ] <- (simulated_results[,i]$Beta_est_bSplines - Beta_true - Beta_true)**2
  MSEs_PH_[i, ] <- (simulated_results[,i]$Beta_est_PH - Beta_true - Beta_true)**2
...
plot(Variance_exponential, type = "o", col = "blue", xlab = "estimated Betas", ylab = "Variance", main = "Variance for Beta estimation", pch = 16, ylim = c(0, 0.3))
...
# lines(MSEs_PH, type = "o", col = "purple", pch = 16)
# 
# abline(v = 10, col = "red", lty = 2)
# legend("topright", legend = c("Exponential", "Weibull", "B-Splines", "ParticalLiklihood"), col = c("blue", "green", "orange", "purple"), pch = 16, lty = 1) # Add legend
...
boxplot(FDR_list, names = c("Exponential", "Weibull", "B-Splines", "ParticalLiklihood"), col = c("blue", "green", "orange", "purple"), main = "FDR Boxplots", xlab = "", ylab = "FDR", ylim = c(0, 1))
abline(h = 0.05, col = "red", lty = 2)

# Subfigure 5 Type II error
Num_Selected_list = list(Exponential = Num_Selected_exponential/sum(Beta_true != 0), Weibull = Num_Selected_weibull/sum(Beta_true != 0), BSplines = Num_Selected_bSplines/sum(Beta_true != 0), ParticalLiklihood = Num_Selected_PH/sum(Beta_true != 0))
boxplot(Num_Selected_list, names = c("Exponential", "Weibull", "B-Splines", "ParticalLiklihood"), col = c("blue", "green", "orange", "purple"), main = "Selected variables Boxplots", xlab = "", ylab = "Selected variables", ylim = c(0, 1))
