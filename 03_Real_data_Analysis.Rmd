...
strafied_Data_DBD <- subset(real_df, real_df$TypecadavericDBDDCD == "Donation after brain death")
strafied_Data_DCD <- subset(real_df, real_df$TypecadavericDBDDCD == "Donation after circulatory death")

strafied_Data_DBD <- subset(strafied_Data_DBD, select = -c(Typeofdonor, InitialPrimaryDiseaseET, RecipientfullmatchHLAstring, TypecadavericDBDDCD))

strafied_Data_DCD <- subset(strafied_Data_DBD, select = -c(Typeofdonor, InitialPrimaryDiseaseET, RecipientfullmatchHLAstring, TypecadavericDBDDCD))
...
includingInter <- function(dataframe_) {
  ...
  interaction_formula <- as.formula(paste("~ (", paste(colnames(predictors), collapse = " + "), ")^2 - 1"))
  ...
  interaction_df$status = dataframe_$status
  interaction_df$obstime = interaction_df$time
...
interaction_strafied_Data_DBD <- includingInter(strafied_Data_DBD)
stan_data_bSplines_DBD <- stan_data_Constructer(
      training_dataset = interaction_strafied_Data_DBD,
...
model_result_bSplines_DBD <- Bayesian_Survival_result_Extract(bayesian_model_fit = diagnostics_bSplines_DBD, model_type = "bSplines", criteria = c("DesignCoefficients", "baseline", "variableSelection"))
...
obs_window = 3415 #max(df_using$time_tx_date_max2)
# only for estimating the Betas
interaction_strafied_Data_DCD <- includingInter(strafied_Data_DCD)
stan_data_bSplines_DCD <- stan_data_Constructer(
      training_dataset = interaction_strafied_Data_DCD,
      baseline_modelling = "bSplines",
      obs_window = obs_window
    )
...
#model diagnotics
diagnostics_bSplines_DCD <- summary(model_fit_bSplines)$summary[, c("Rhat", "n_eff")]
...
FDR_beta_DCD = model_result_bSplines_DCD$variableSelection
```
