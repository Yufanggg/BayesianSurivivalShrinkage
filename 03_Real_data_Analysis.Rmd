...
cbind(mean(CI_index_PL, CI_index_bSplines))
...
Data_DBD_PL_Int_full <- stan_data_Constructer_PL(training_dataset = Data_DBD_bSplines_Int_full,
                                            testing_dataset = NULL,
                                            havingInt = TRUE)
...
PL_model_Int_results_full = Bayesian_Survival_result_Extract(
  ...
  model_type = "bSplines",
  criteria = c("DesignCoefficients", "variableSelection", "Prediction_SurvivalProb"))
PL_model_Int_full_coef <- strafied_Data_DBD_int$Beta_bayesian_est[, "mean"]
  

# Initialize residuals
schoenfeld_res_bSpline <- rep(NA, length(strafied_Data_DBD_int$status))
schoenfeld_res_PL<- rep(NA, length(strafied_Data_DBD_int$status))

# Loop through each event time
for (i in which(strafied_Data_DBD_int$status == 1)) {
  t_i <- strafied_Data_DBD_int$time[i]
  x_i <- strafied_Data_DBD_int_full[i, -c("time", "status")]
  
  # Risk set: those still at risk at time t_i
  risk_set <- which(strafied_Data_DBD_int$time >= t_i)
  X_risk <- X[risk_set, ]
  
  # Compute weights for expected value
  weights_bSpline <- exp(X_risk %*% bSplines_model_Int_full_coef)
  weights_PL <- exp(X_risk %*% PL_model_Int_full_coef)
  ...
  schoenfeld_res_bSpline[i, ] <- x_i - expected_x_bSpline
  schoenfeld_res_PL[i, ] <-  x_i - expected_x_PL
}
...
axis(1, at = 1:91, labels = names(strafied_Data_DBD_int_noise)[1:91], las = 2, cex.axis = 0.7)
