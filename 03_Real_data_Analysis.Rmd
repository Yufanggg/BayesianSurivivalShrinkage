...
```{r, echo=TRUE}
real_df <- readRDS("./Data/imputed_NOTR_DGF_2.rds")
...
real_df$Donorcauseofdeath_group <- ifelse(real_df$Donorcauseofdeath_group == "Cardiac", 1, 0)
real_df$InitialPrimaryDiseaseET_regroup <- ifelse(real_df$InitialPrimaryDiseaseET_regroup == "Diabetes Mellitus", 1, 0)
...
# DBD data
```{r, echo=TRUE}
#Get the frequency table for DBD
DBD_Frequency_tab <- data.frame(
  c = character(),
  Levels = integer(),
  MissingObs = integer(),
  UniqueValues = character()
)
factorName["TypecadavericDBDDCD"] <- FALSE
FactorVariables <- colnames(real_df)[factorName]


for (facvar in FactorVariables){
  #print(facvar)
  Freq_tab = table(strafied_Data_DBD[[facvar]])
  print(Freq_tab)
  categories_Name = names(Freq_tab)
  #Categories_Freq = table(Freq_tab)
  temp_df <- data.frame(
    Variable = facvar,
    NumLevels = length(unique(strafied_Data_DBD[[facvar]])),
    factor_Name = paste(categories_Name, collapse = ","),
    factor_Freq = paste(Freq_tab, collapse = ",")
  )
  
  DBD_Frequency_tab <- rbind(DBD_Frequency_tab, temp_df)
  
  temp_df <- data.frame(); rm(Freq_tab, categories_Name)
}

DBD_Frequency_tab

write.csv(DBD_Frequency_tab, "./Data/DBD_Frequency.csv", row.names = FALSE)
str(strafied_Data_DBD)


#Get the frequency table for DCD
DCD_Frequency_tab <- data.frame(
  c = character(),
  Levels = integer(),
  MissingObs = integer(),
  UniqueValues = character()
...
for (facvar in FactorVariables){
  #print(facvar)
  Freq_tab = table(strafied_Data_DCD[[facvar]])
  print(Freq_tab)
  categories_Name = names(Freq_tab)
  #Categories_Freq = table(Freq_tab)
  temp_df <- data.frame(
    Variable = facvar,
    NumLevels = length(unique(strafied_Data_DCD[[facvar]])),
    factor_Name = paste(categories_Name, collapse = ","),
    factor_Freq = paste(Freq_tab, collapse = ",")
  )
  
  DCD_Frequency_tab <- rbind(DCD_Frequency_tab, temp_df)
...
write.csv(DBD_Frequency_tab, "./Data/DCD_Frequency.csv", row.names = FALSE)
str(strafied_Data_DCD)

save(strafied_Data_DBD, strafied_Data_DCD, file = "./Data/strafied_Data.RData")
```
...
model_fit_bSplines_DBD <- Bayesian_Survival_model(stan_data = stan_data_bSplines_DBD, withPrediction = FALSE, baseline_modelling = "bSplines", shrinkage = TRUE)

#model diagnotics
diagnostics_bSplines_DBD <- summary(model_fit_bSplines_DBD)$summary[, c("Rhat", "n_eff")]

print(diagnostics_bSplines_DBD)
...
save.image(file = "./Data/03_results_DBD_2.RData")
```
...
model_fit_bSplines_DCD <- Bayesian_Survival_model(stan_data = stan_data_bSplines_DCD, withPrediction = FALSE, baseline_modelling = "bSplines")
...
cross_val_metric_DCD <- cross_validation(whole_dataset = strafied_Data_DCD_int, baseline_modelling = "bSplines")
save.image(file = "./Data/03_results_DCD_2.RData")
```
