...
bSplines_model_Int_full <- Bayesian_Survival_model(
  stan_data = Data_DBD_bSplines_Int_full,
  baseline_modelling = "bSplines",
  shrinkage = TRUE,
  withPrediction = TRUE,
  havingInt = TRUE
...
for (i in which(strafied_Data_DBD_int$status == 1)) {
  ...
  x_i <- strafied_Data_DBD_int[i, -c("time", "status")]
  
  # Risk set: those still at risk at time t_i
  risk_set <- which(strafied_Data_DBD_int$time >= t_i)
  
  # Compute weights for expected value
  weights_bSpline <- exp(bSplines_model_Int_full_coef %*% X[risk_set])
  
  # Expected covariate values
  expected_x_bSpline <- colSums(X_risk * weights) / sum(weights)
  
  
  # Schoenfeld residuals
  schoenfeld_res_bSpline[i, ] <- x_i - expected_x
}


# View residuals for events
schoenfeld_res[!is.na(schoenfeld_res[,1]), ]
...
#---PL model
Data_DBD_PL_Int_full <- stan_data_Constructer_PL(
  training_dataset = strafied_Data_DBD_int_full,
  testing_dataset = NULL,
  havingInt = TRUE
)

PL_model_Int_full <- Bayesian_Survival_model(
  stan_data = Data_DBD_PL_Int_full,
  baseline_modelling = "none",
  shrinkage = TRUE,
  withPrediction = TRUE,
  havingInt = TRUE,
