...
simulation_bayint_survival <- function(baseline_modelling = "weibull", criteria = "DesignCoefficients"){
  simulated_data <- DataGenerator(n_samples = 500, n_features = 10)
  dataset <- simulated_data$dataset
  Beta <- simulated_data$Beta
  
  stan_data <- stan_data_Constructer(training_dataset = dataset, testing_dataset = head(dataset, 10), baseline_modelling = baseline_modelling, obs_window = 5)
  ...
  model_result <- Bayesian_Survival_result_Extract(bayesian_model_fit = model_fit, criteria = criteria)
  
  output = list(
    Beta_true = Beta,
    Beta_est = model_result$Beta_bayesian_est[, "mean"]
  )
  
  return(output)
}

simulated_result <- simulation_bayint_survival(baseline_modelling = "weibull")
