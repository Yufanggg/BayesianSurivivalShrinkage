...
simulation_bayint_survival <- function(criteria = c("DesignCoefficients", "baseline", "variableSelection"), withPrediction = TRUE){
  output = list()
...
baseline_exponential = matrix(nrow = replicate_times, ncol = 200)
baseline_weibull = matrix(nrow = replicate_times, ncol = 200)
baseline_bSplines = matrix(nrow = replicate_times, ncol = 200)
...
for (i in 1:replicate_times){
  ...
  baseline_bSplines[i, ] <- as.vector(simulated_results[,i]$baseline_bSplines)
}
