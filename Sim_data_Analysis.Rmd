...
simulation_bayint_survival <- function(criteria = c("DesignCoefficients", "baseline"), withPrediction = FALSE){
  output = list()
  ...
  if ("baseline" %in% criteria) {
    time_vec <- sort(dataset$obstime) # obs_window = 5
    output$baseline_exponential <- rep(model_result_exponential$baselinePara, length(time_vec))
    
    weibull_lambda <- model_result_weibull$baselinePara[1]
    weibull_shape <- model_result_weibull$baselinePara[2]
    output$baseline_weibull <- weibull_shape * weibull_lambda * (time_vec**(weibull_shape -1))
      
    output$baseline_bSplines <- exp(as.matrix(stan_data_bSplines$basis) %*% as.matrix(model_result_bSplines$baselinePara[, "mean"]))
  }
  
  
  if ("variableSelection" %in% criteria){
    output$variableSelection_exponential <- model_result_exponential$variableSelection
    output$variableSelection_weibull <- model_result_weibull$variableSelection
    output$variableSelection_bSplines <- model_result_bSplines$variableSelection
  }
...
save.image(file = "sim_50_para_int.RData")
```
...
replicate_times = 50
time_vec = time_vec
baseline_exponential = matrix(nrow = replicate_times, ncol = time_vec)
baseline_weibull = matrix(nrow = replicate_times, ncol = time_vec)
baseline_bSplines = matrix(nrow = replicate_times, ncol = time_vec)
...
for (i in 1:replicate_times){
  ...
  baseline_exponential[i, ] <- simulated_results[,i]$baseline_exponential
  baseline_weibull[i, ] <- simulated_results[,i]$baseline_weibull
  baseline_bSplines[i, ] <- simulated_results[,i]$baseline_bSplines
}
...
mean_baseline_exponential <- apply(baseline_exponential, 2, mean)
mean_baseline_weibull <- apply(baseline_weibull, 2, mean)
mean_baseline_bSplines <- apply(baseline_bSplines, 2, mean)
...
plot(mean_baseline_exponential, col = "green")
lines(mean_baseline_weibull, col = "orange")
lines(mean_baseline_bSplines, col = "blue")
# Add legend
legend("topright", legend = c("Exponential", "Weibull", "B-Splines"), col = c("blue", "green", "orange"), pch = 16, lty = 1)
...
```{r, echo=TRUE}
a = simulated_results[, 1]
dim(a$baseline_bSplines)

plot(a$baseline_weibull, col = "green")
lines(a$baseline_bSplines, col = "orange")
lines(a$baseline_exponential, col = "blue")
# Add legend
legend("topright", legend = c("Exponential", "Weibull", "B-Splines"), col = c("blue", "green", "orange"), pch = 16, lty = 1)
```
