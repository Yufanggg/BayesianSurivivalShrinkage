...
# function for one simulationc
simulation_bayint_survival <- function(criteria = c("DesignCoefficients", "baseline"), withPrediction = TRUE){
  ...
  if ("baseline" %in% criteria) {
    model_result_bSplines$baseline <- model_result_exponential$baselinePara
    
    weibull_lambda <- model_result_weibull$baselinePara[1]
    weibull_shape <- model_result_weibull$baselinePara[2]
    time_vec <- seq(0, 5, 0.1) # obs_window = 5
    model_result_weibull$baseline <- weibull_shape * weibull_lambda * (time_vec**(weibull_shape -1))
      
    bSpline_basline <- exp(model_result_bSplines$baselinePara %*% stan_data_bSplines$basis)
    model_result_bSplines$baseline <- bSpline_basline
  }
