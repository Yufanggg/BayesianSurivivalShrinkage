Data_proprocessing1 <- function() {
  ...
  NOTR_DGF.df <-
    read_excel("../Data/NOTR2.xlsx",
               sheet = 'NOTR data alle centra_20230509_',
               guess_max = 50000)
  ...
  NOTR_DGF.df <- NOTR_DGF.df %>%
    mutate(
      ...
      PT_Earlydeath = if_else(PT_surv_time < 10 &
                                PT_Death == 1, 1, 0),
  ...
  NOTR_DGF.df <- NOTR_DGF.df %>%
    dplyr::rename(
      "Number_of_transplantations" = "TransplantKidneyordernrorgan",
  ...
  NOTR_DGF.df <- NOTR_DGF.df %>%
    select(-Transplantmethod,
           -Lowestcreatininemgdl,
           -Highestcreatininemgdl)
  ...
  NOTR_DGF.df <- NOTR_DGF.df %>%
    mutate(
      ...
      Lowestcreatinine ?  ? moll = case_when(
        Lowestcreatinine ?  ? moll < 1001 ~ Lowestcreatinine ?  ? moll,
        Lowestcreatinine ?  ? moll > 1000 ~
          NA
      ),
  ...
  NOTR_DGF.df$DonorBMI <-
    NOTR_DGF.df$Donorweight / ((NOTR_DGF.df$Donorheight / 100) ^ 2)
  NOTR_DGF.df$Recipient_BMI <-
    NOTR_DGF.df$Recipient_weight / ((NOTR_DGF.df$Recipient_height / 100) ^
                                      2)
  NOTR_DGF.df$Recipient_BMI <-
    ifelse(
      NOTR_DGF.df$Recipient_BMI > 16 &
        NOTR_DGF.df$Recipient_BMI < 60,
      NOTR_DGF.df$Recipient_BMI,
      NA
    )
  NOTR_DGF.df$DonorBMI <-
    ifelse(NOTR_DGF.df$DonorBMI > 16 &
             NOTR_DGF.df$DonorBMI < 60,
           NOTR_DGF.df$DonorBMI,
           NA)
  NOTR_DGF.df$Year_Of_Tx = NOTR_DGF.df$Dateoftransplant
  NOTR_DGF.df$Year_Of_Tx <-
    as.numeric(format(NOTR_DGF.df$Year_Of_Tx, format = "%Y"))
  ...
  circulatory_conditions <- c(
    "Circulational: Acute Myocard Infarct",
  ...
  bcva_conditions <- c("CVA: Intra Cerebral Bleeding",
  ...
  respiratory_conditions <-
    c(
      "Respirational: Epiglotitis / Laryngitis",
  ...
  NOTR_DGF.df <-
    ...
    mutate(last_FU_date = case_when(
      !is.na(Recipientdateofdeath) ~ ymd(Recipientdateofdeath),
      is.na(Dateseen.END) &
        is.na(Recipientdateofdeath) ~ caseDay,
      is.na(Recipientdateofdeath) &
        !is.na(Dateseen.END) ~ ymd(Dateseen.END)
      #
  ...
  NOTR_DGF.df <-
    NOTR_DGF.df %>%
    mutate(Follow_up_time = as.numeric(difftime(
      last_FU_date, Dateoftransplant1,  units = 'weeks'
    )) /
      52.25) %>%
    mutate(DGF_duration = difftime(ymd(DGFLastdialysisdate), ymd(Dateoftransplant), units = 'days')) %>%
    mutate(GraftLoss = if_else(
      is.na(InitialGraftfaildate) &
        is.na(InitialGraftfailcause),
      0,
      1
    ))
  ...
  NOTR_DGF.df <-
    ...
    mutate(
      PNF = case_when(
        InitialGraftfailcause ==
          "Patient died with functioning transplant" ~ 0 ,
        TRUE ~ PNF
      )
    )
  ...
  NOTR_DGF.df <-
    NOTR_DGF.df %>% mutate(dDGF = relevel(as.factor(dDGF), ref = "0")) %>%
  ...
  NOTR_DGF.df <- NOTR_DGF.df %>%
    mutate(
      Donor_type_deceased = case_when(
        Donor_type_deceased == 'Donation after brain death' ~ "DBD",
  ...
  NOTR_DGF.df$time_tx_death <-
    round(
      difftime(
        NOTR_DGF.df$Recipientdateofdeath,
        NOTR_DGF.df$Dateoftransplant,
        units = "days"
      )
    )
  
  NOTR_DGF.df$time_tx_graftloss <-
    round(
      difftime(
        NOTR_DGF.df$InitialGraftfaildate,
        NOTR_DGF.df$Dateoftransplant,
  ...
  NOTR_DGF.df$time_tx_date_max <-
    round(difftime(
      NOTR_DGF.df$last_FU_date,
      NOTR_DGF.df$Dateoftransplant,
      units = "days"
    ))
  
  NOTR_DGF.df$time_tx_date_max2 <-
    round(difftime(date("2023-05-09"), NOTR_DGF.df$Dateoftransplant, units =
                     "days"))
  
  
  NOTR_DGF.df$diff_tx_death <-
    NOTR_DGF.df$time_tx_graftloss - NOTR_DGF.df$time_tx_death
  
  NOTR_DGF.df$status <- NA
  NOTR_DGF.df$status <-
    ifelse(!is.na(NOTR_DGF.df$InitialGraftfaildate) |
             NOTR_DGF.df$PNF == 1,
           "graftloss",
           NA)
  NOTR_DGF.df$status <-
    ifelse(
      !is.na(NOTR_DGF.df$diff_tx_death) &
        NOTR_DGF.df$diff_tx_death >= 0 ,
      "death",
      NOTR_DGF.df$status
    )
  NOTR_DGF.df$status <-
    ifelse(
      NOTR_DGF.df$InitialGraftfailcause == "Patient died with functioning transplant",
      "death",
      NOTR_DGF.df$status
    )
  NOTR_DGF.df$status <-
    ifelse(is.na(NOTR_DGF.df$status), "alive", NOTR_DGF.df$status)
  
  NOTR_DGF.df$time <-
    ifelse(NOTR_DGF.df$status == "graftloss",
           NOTR_DGF.df$time_tx_graftloss,
           NA)
  NOTR_DGF.df$time <-
    ifelse(NOTR_DGF.df$status == "death",
           NOTR_DGF.df$time_tx_death,
           NOTR_DGF.df$time)
  NOTR_DGF.df$time <-
    ifelse(NOTR_DGF.df$status == "alive",
           NOTR_DGF.df$time_tx_date_max2,
           NOTR_DGF.df$time)
  ...
  NOTR_DGF.df$status2 <-
    ifelse(NOTR_DGF.df$status == "graftloss", 1, NOTR_DGF.df$status2)
  NOTR_DGF.df$status2 <-
    ifelse(NOTR_DGF.df$status == "alive", 0, NOTR_DGF.df$status2)
  # NOTR_DGF.df$status2 <- as.factor(NOTR_DGF.df$status2)
  
  
  # Check if all recipients with graft loss date are labeled correctly
  table(is.na(NOTR_DGF.df$InitialGraftfaildate),
        NOTR_DGF.df$status,
        exclude = NULL)
  ...
  columns_eGFR <-
    names(NOTR_DGF.df)[grepl("Creatinine", names(NOTR_DGF.df), ignore.case = FALSE)]
  ...
  # back to wide format
  NOTR_DGF.df.egfr.short <-
    spread(NOTR_DGF.df.egfr[, !names(NOTR_DGF.df.egfr) %in% c("fictiefrecipientnummer")], "day", "eGFR")
