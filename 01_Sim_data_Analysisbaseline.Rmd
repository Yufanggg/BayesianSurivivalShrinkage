...
simulation_bayint_survival <- function(criteria = c("DesignCoefficients", "baseline", "variableSelection", "Prediction_SurvivalProb"), withPrediction = TRUE, n_samples = n_samples, n_features = n_features, event_prop = 0.75){
  ...
  stan_data_PH <- stan_data_Constructer_PL(
  ...
  if ("baseline" %in% criteria) {
    if (withPrediction){
      output$time_vec <-
      sort(unique(training_dataset$obstime))#[training_dataset$status == 1, ]$obstime) # obs_window = 5
    } else {
       output$time_vec <-
         sort(unique(dataset$obstime)) #sort(dataset[dataset$status == 1, ]$obstime) # obs_window = 5
    }
    ...
    # output$baseline_PL <- compute_baseline_hazard(beta = output$Beta_est_PH, X = as.matrix(training_dataset[, !(colnames(training_dataset) %in% c("id", "status", "obstime"))]), time = output$time_vec, status = training_dataset$status)
    # 
...
lines(Bias_weibull, type = "o", col = "green", pch = 15)
lines(Bias_bSplines, type = "o", col = "orange", pch = 17)
lines(Bias_PH, type = "o", col = "purple", pch = 18)

abline(v = 10, col = "red", lty = 2, lwd = 3)
legend("topright", legend = c("Exponential", "Weibull", "B-Splines", "ParticalLiklihood"), col = c("blue", "green", "orange", "purple"), pch = c(16, 15, 17, 18), lty = 1) # Add legend
...
lines(Variance_weibull, type = "o", col = "green", pch = 15)
lines(Variance_bSplines, type = "o", col = "orange", pch = 17)
lines(Variance_PH, type = "o", col = "purple", pch = 18)
legend("topright", legend = c("Exponential", "Weibull", "B-Splines", "ParticalLiklihood"), col = c("blue", "green", "orange", "purple"), pch = c(16, 15, 17, 18), lty = 1)# Add legend
abline(v = 10, col = "red", lty = 2, lwd = 3)

# Subfigure 3: MSE
plot(rMSEs_exponential, type = "o", col = "blue", xlab = "estimated Betas", ylab = "rMSE", main = "rMSE for Beta estimation", ylim = c(0, 0.5), pch = c(16, 15, 17, 18))
...
lines(rMSEs_weibull, type = "o", col = "green", pch = 15)
lines(rMSEs_bSplines, type = "o", col = "orange", pch = 17)
lines(rMSEs_PH, type = "o", col = "purple", pch = 18)
abline(v = 10, col = "red", lty = 2, lwd = 3)
...
boxplot(CI_index_list, names = c("Exponential", "Weibull", "B-Splines", "ParticalLiklihood"), col = c("blue", "green", "orange", "purple"), main = "C index", xlab = "", ylab = "C index", ylim = c(.5, 1))
```
...
for (i in 1:replicate_times) {
  ...
  lines(output$time_vec, output$baseline_weibull, col = "green", type = "l", pch = 16, lty = 1, lwd = 2)
  lines(output$time_vec, output$baseline_exponential, col = "blue", pch = 16, lty = 1, lwd = 2)
  #lines(output$aseline_PL, col = "black", pch = 16, lty = 1, lwd = 2)
  lines(output$time_vec, output$baseline_bSplines, col = "orange", pch = 16, lty = 1, lwd = 2)
  # Add legend
  legend(
    "topleft",
