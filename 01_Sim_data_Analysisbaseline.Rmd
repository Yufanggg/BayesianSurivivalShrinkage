...
simulation_bayint_survival <- function(criteria = c("DesignCoefficients", "baseline", "variableSelection", "Prediction_SurvivalProb"), withPrediction = TRUE){
  ...
  #fit model
  model_fit_exponential <-  Bayesian_Survival_model(stan_data = stan_data_exponential, withPrediction = TRUE, baseline_assumption = "exponential", shrinkage = TRUE)
  model_fit_weibull <- Bayesian_Survival_model(stan_data = stan_data_weibull, withPrediction = TRUE, baseline_assumption = "weibull", shrinkage = TRUE)
  model_fit_bSplines <- Bayesian_Survival_model(stan_data = stan_data_bSplines, baseline_assumption = "bSplines", shrinkage = TRUE)
  ...
  if ("Prediction_SurvivalProb" %in% criteria) {
    ...
    output$eta_true = as.vector(as.matrix(stan_data_weibull$x_new) %*% output$Beta_true[1:10] + as.matrix(stan_data_weibull$x_int_new) %*% output$Beta_true[11:55])
...
boxplot(FDR_list, names = c("exponential", "weibull", "bSplines"), col = c("blue", "green", "orange"), main = "FDR Boxplots", xlab = "", ylab = "FDR")

# Create the Brier score
cor_list = list(exponential = cor_exponential, weibull = cor_weibull, bSplines = cor_bSplines)
boxplot(cor_list, names = c("exponential", "weibull", "bSplines"), col = c("blue", "green", "orange"), main = "True vs. predicted linear predictor", xlab = "", ylab = "correlation")

cor_sp_list = list(cor_sp_exponential = cor_sp_exponential, cor_sp_weibull = cor_sp_weibull, cor_sp_bSplines = cor_sp_bSplines)
boxplot(cor_sp_list, names = c("exponential", "weibull", "bSplines"), col = c("blue", "green", "orange"), main = "cor_sp Boxplots", xlab = "", ylab = "cor_sp")
...
boxplot(CI_index_list, names = c("exponential", "weibull", "bSplines"), col = c("blue", "green", "orange"), main = "C index Boxplots", xlab = "", ylab = "C index")
```
