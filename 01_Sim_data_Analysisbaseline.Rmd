...
for (i in 1:replicate_times){
  ...
  CI_index_exponential[i] <- Cindex(Surv(time, true_status), 1- sp_pre_exponential, time) # rcorr.cens
  CI_index_weibull[i] <- Cindex(Surv(time, true_status), 1-sp_pre_weibull, time)
  CI_index_bSplines[i] <- Cindex(Surv(time, true_status), 1-sp_pre_bSplines, time)
}
