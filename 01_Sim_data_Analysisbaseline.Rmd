...
required_packages <- c("rstan", "MASS", "rstantools", "coda", "readr", "survival", "splines2", "dplyr", "simsurv", "SurvMetrics", "caret", "foreach", "doParallel")
...
for (i in 1:replicate_times){
  ...
  Brier_score_exponential[i] <- mean((true_status - sp_pre_exponential)**2)
  Brier_score_weibull[i] <- mean((true_status -  sp_pre_weibull)**2)
  Brier_score_bSplines[i] <- mean((true_status - sp_pre_bSplines)**2)
  
  # CI 
  time <- simulated_results[,i]$time
  CI_index_exponential[i] <- rcorr.cens(eta_pre_exponential, Surv(time, true_status))["concordant"] # rcorr.cens
  CI_index_weibull[i] <- rcorr.cens(eta_pre_weibull, Surv(time, true_status))["concordant"]
  CI_index_bSplines[i] <- rcorr.cens(eta_pre_bSplines, Surv(time, true_status))["concordant"]
}
