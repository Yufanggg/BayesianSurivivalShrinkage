...
for (i in 1:replicate_times){
  ...
  CI_index_exponential[i] <- rcorr.cens(Surv(time, true_status) ~ eta_pre_exponential) # rcorr.cens
  CI_index_weibull[i] <- rcorr.cens(Surv(time, true_status) ~ eta_pre_weibull)
  CI_index_bSplines[i] <- rcorr.cens( Surv(time, true_status) ~ eta_pre_bSplines[seq(15, length(eta_pre_bSplines), by = 15)])
}
